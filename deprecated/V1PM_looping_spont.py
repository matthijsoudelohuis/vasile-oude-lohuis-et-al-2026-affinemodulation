...
session_list            = np.array([['LPE09665_2023_03_14',
                                     'LPE09665_2023_03_21',
                                        'LPE09830_2023_04_10',
                                        'LPE10885_2023_10_12',
                                        'LPE10885_2023_10_19',
...
for ises in tqdm(range(nSessions),total=nSessions,desc='Computing corr rates and affine mod'):
    ...
    [N,K]           = np.shape(respdata) #get dimensions of response matrix
    ...
    for ialp,alp in enumerate(arealabelpairs):
        ...
        idx_K1 = np.where(meanpopact < np.nanpercentile(meanpopact,params['splitperc']))[0]
        idx_K2 = np.where(meanpopact > np.nanpercentile(meanpopact,100-params['splitperc']))[0]
        ...
        if ndprimeboots:
            ...
            for iboot in range(ndprimeboots):
                idx_K1              = np.random.choice(np.arange(len(meanpopact)),size=len(meanpopact)*params['splitperc']//100,replace=False)
                idx_K2              = np.random.choice(np.arange(len(meanpopact)),size=len(meanpopact)*params['splitperc']//100,replace=False)
...
my_savefig(fig,savedir,'Looped_Modulations_Spontaneous_Bootstrap_Radii_dprime')
...
plt.tight_layout()
sns.despine(fig=fig, top=True, right=True,offset=3)
my_savefig(fig,savedir,'Looped_Dprime_Modulations_Spontaneous_Bootstrap_Radius%d' % (radius))
